...
LEFT JOIN
    title_principals t
ON
    a."imdb_name_id" = t."imdb_name_id"
LEFT JOIN
    movies m
ON
    t."imdb_title_id" = m."imdb_title_id"
WHERE name
          IN('Morgan Freeman', 'Tom Hanks')
...
WHERE characters ILIKE '%James Bond%'
GROUP BY 1
...
SELECT
    ...
    females_18age_avg_vote-males_18age_avg_vote AS f_m_differenc
FROM movies m
LEFT JOIN ratings r
ON m.imdb_title_id = r.imdb_title_id
WHERE
      females_18age_votes >=10000 AND males_18age_votes >= 10000
ORDER BY f_m_differenc DESC, original_title
LIMIT 5;
...
SELECT
    ...
    males_18age_avg_vote,
    females_18age_avg_vote,
    males_18age_avg_vote-females_18age_avg_vote AS m_f_differenc
FROM movies m
LEFT JOIN ratings r
ON m.imdb_title_id = r.imdb_title_id
WHERE
      males_18age_votes >=10000 AND females_18age_votes >= 10000
ORDER BY m_f_differenc DESC, original_title
LIMIT 5;
...
SELECT
    ...
    (females_45age_avg_vote + males_45age_avg_vote)/2 AS avg_vote_45plus,
    ((females_45age_avg_vote + males_45age_avg_vote)/2)- ((males_18age_avg_vote + females_18age_avg_vote)/2)AS delta
FROM movies m
LEFT JOIN ratings r
ON m.imdb_title_id = r.imdb_title_id
WHERE males_18age_votes>=10000 AND females_18age_votes >= 10000 AND males_45age_votes >=10000 AND females_45age_votes >= 10000
ORDER BY delta DESC, original_title
LIMIT 5;
...
ON r.imdb_title_id = tp.imdb_title_id
WHERE category ='actress' AND genre LIKE '%Drama%'
GROUP BY 1
...
SELECT
   a.name,
COUNT(CASE WHEN m.avg_vote >= 8 THEN m.title ELSE NULL END) AS num_8_plus_movies,
COUNT(m.title) num_total_movies,
ROUND(COUNT(CASE WHEN m.avg_vote>=8 THEN m.title else null end)/CAST(COUNT(m.title) AS DECIMAL)*100, 1) AS pct_8plus_movies
FROM actors a
INNER JOIN title_principals tp
ON tp.imdb_name_id = a.imdb_name_id
INNER JOIN (SELECT * FROM movies WHERE votes>=10000 ) m on tp.imdb_title_id = m.imdb_title_id
WHERE tp.category = 'actress' OR tp.category = 'actor'
GROUP BY 1
...
SELECT
    original_title,
    COUNT(a.name) AS num_child_actors

FROM actors a
JOIN title_principals tp ON a.imdb_name_id = tp.imdb_name_id
JOIN movies m ON m.imdb_title_id = tp.imdb_title_id
WHERE date_of_birth LIKE '____-__-__'
    AND date_published LIKE '____-__-__'
AND DATE_PART('year', AGE(DATE(m.date_published), DATE(a.date_of_birth)))<18
      --age(date(date_published), date(date_of_birth)) < '18 years 0 mons 00 days'
AND votes >= 10000
GROUP BY 1
ORDER BY 2 DESC,1
LIMIT 10;
